...
import ModalAddJamIstirahat from "./ModalAddJamIstirahat";

const Layout = () => {
  const [jamIstirahat, setJamIstirahat] = useState([]);
  ...
  const _jamIstirahat = async () => {
    setTableLoading(true);
    setMessage("");

    try {
      const res = await axios.get(`${API_URL}/jadwal-jam-istirahat`, {
        params: {
          prodi: prodiAdmin,
        },
      });
  ...
  return (
    <>
    <ModalAddJamIstirahat
      isOpen={showModal} handleClose={handleCloseModal} onSuccess={_jamIstirahat}
    />
      <div>
        <h2 className="text-2xl font-semibold mb-4 dark:text-white">Jam Istirahat</h2>

        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
          <div className="flex gap-2">
            <Button
              text="Add Jam Istirahat"
        ...
        <div className="mt-5 overflow-x-auto bg-white dark:bg-[#282828] rounded-xl p-4 shadow-md relative">
          ...
          {jamIstirahat.length === 0 && !tableLoading && !message ? (
              ...
              <table className="min-w-full divide-y divide-gray-300 dark:divide-[#3f3f3f]">
                <thead>
                  <tr className="text-sm dark:text-white">
                    ...
                    <th className="px-4 py-3 text-left font-medium">Jam Istirahat</th>
                    <th className="px-4 py-3 text-left font-medium">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-[#3f3f3f]">
                  {jamIstirahat.map((jamIstirahat, index) => (
                    <tr
                      key={jamIstirahat.id}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="px-2 py-1 text-xs font-medium bg-purple-100 text-purple-800 rounded-full">
                          {jamIstirahat.jamMulaiIstirahat} - {jamIstirahat.jamSelesaiIstirahat}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex gap-2">
                          <ButtonAction
                            onClick={() => handleDelete(jamIstirahat.id)}
