...
import ModalAddMataKuliah from "./ModalAddMataKuliah";
...
const Layout = () => {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${API_URL}/matkul/${id}`);
          getJadwal();
  ...
  return (
    <>
    <ModalAddMataKuliah
      isOpen={showModal} handleClose={handleCloseModal} onSuccess={getJadwal}
    />
      <div>
        <h2 className="text-2xl font-semibold mb-4 dark:text-white">Mata Kuliah</h2>

        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
          <div className="flex gap-2">
            <Button
              text="Add Mata Kuliah"
              ...
              <table className="min-w-full divide-y divide-gray-300 dark:divide-[#3f3f3f]">
                <thead>
                  <tr className="text-sm dark:text-white">
                    ...
                    <th className="px-4 py-3 text-left font-medium">Kode Mata Kuliah</th>
