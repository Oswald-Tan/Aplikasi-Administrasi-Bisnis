...
const Layout = () => {
  ...
  useEffect(() => {
    // Hanya panggil getUsers jika authUser sudah tersedia
    if (authUser && authUser.prodiAdmin) {
      getUsers();
    } else if (authUser && !authUser.prodiAdmin) {
      // Hanya set pesan jika authUser sudah ada tapi tidak punya prodiAdmin
  ...
  const getUsers = async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/mahasiswa`, {
        params: {
          search: keyword,
          page: page,
          limit: limit,
  ...
  return (
    <div>
      <h2 className="text-2xl font-semibold mb-4 dark:text-white">
        Mahasiswa - Prodi {authUser?.prodiAdmin || "Belum dikonfigurasi"}
      </h2>
                ...
                {users.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex gap-2">
                        <ButtonAction
                          to={`/users/mahasiswa/edit/${user.id}`}
                          icon={<MdEditSquare size={16} />}
                          className="bg-blue-600 hover:bg-blue-700"
                          tooltip="Edit"
                        />
                        <ButtonAction
                          to={`/users/mahasiswa/${user.id}/details`}
                          icon={<BiSolidUserDetail size={16} />}
